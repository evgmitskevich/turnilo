...
export enum MeasurePercentOf { PARENT = "of_parent", TOTAL = "of_total" }
...
export class DataSeries extends Record<DataSeriesValue>(defaultDataSeries) {
  ...
  static namePattern = new RegExp(`^(_(${SeriesDerivation.PREVIOUS}|${SeriesDerivation.DELTA})__)?(.+)(__(${MeasurePercentOf.PARENT}|${MeasurePercentOf.TOTAL})_)?$`);

  static nominalName(fullName: string): { name: string, derivation: SeriesDerivation, percentOf?: MeasurePercentOf } {
    ...
    const percentOf = matches[5] ? readMeasurePercentOf(matches[5]) : undefined;
    return { name, derivation, percentOf };
  }

  static fullName(name: string, derivaiton: SeriesDerivation, percentOf?: MeasurePercentOf): string {
    const percentStr = percentOf ? `__${percentOf}_` : "";
    const derivationStr = derivaiton === SeriesDerivation.CURRENT ? "" : `_${derivaiton}__`;
  ...
  private relativeNesting(nestingLevel: number): number {
    switch (this.percentOf) {
      case MeasurePercentOf.TOTAL:
        return Math.min(nestingLevel, 1);
      case MeasurePercentOf.PARENT:
        return nestingLevel;
    }
  }
  ...
  private derivationTitle(derivation: SeriesDerivation): string {
    switch (derivation) {
  ...
  public title(derivation = SeriesDerivation.CURRENT): string {
    const derivationStr = this.derivationTitle(derivation);
