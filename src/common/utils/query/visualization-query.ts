...
import { Essence, PeriodFilters } from "../../models/essence/essence";
...
export default function makeQuery(essence: Essence, timekeeper: Timekeeper): Expression {
  ...
  const filters = essence.getPeriodFilters(timekeeper);

  const mainExp: Expression = ply().apply("main", $main.filter(mainFilter.toExpression(dataCube)));
  const queryWithSeries = applySeries(measureExpressions, 0, filters)(mainExp);

  if (splits.length() > 0) {
    return queryWithSeries.apply(SPLIT, applySplit(0, essence, measureExpressions, filters));
  }
